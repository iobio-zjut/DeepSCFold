...
from utils.model_wio_coratten_step1 import esm_ss_dataset_step1, esm_ss_predict_tri_step1
from utils.model_wio_coratten_step2 import esm_ss_dataset_step2, esm_ss_predict_tri_step2
from utils.util import make_parent_dir
...
def load_pairs_from_txt(filename):
    pairs_list = []
...
def getpickle(query_path, target_path, output_dir, esm_path, embedding_script):
    ...
    features_out_path = os.path.join(output_dir, 'step1_seqs_feature.pkl')
...
def predict_step1(query_path, target_path, pairs_file, output_dir, esm_path, embedding_script, model_path):
    print("---------start predict action score---------")
    print("query: ", query_path)
    if pairs_file:
        print("pairs_file: ", pairs_file)
        pairs_list = load_pairs_from_txt(pairs_file)
    else:
    ...
    pre_model_name = model_path
    print(pre_model_name)
    ...
    predict_pkl_dict_out_path = os.path.join(output_dir, 'predict_seq_pkl_dict.pkl')
    with open(predict_pkl_dict_out_path, 'wb') as output_file:
        pickle.dump(predict_pkl_dict, output_file)

    predict_pkl_dict_out_path = os.path.join(output_dir, 'predict_seq_pkl_dict.pkl')
    with open(predict_pkl_dict_out_path, 'rb') as input_file:
        predict_pkl_dict = pickle.load(input_file)
    ...
    feature_data_file = os.path.join(output_dir, 'step2_pairs_feature.pkl')
    ...
    with torch.no_grad():
        ...
        for i in range(ss_valid_dataset.__len__()):
            ...
            predict_score_dict[query_seqid][target_seqid] = predict_score.cpu().numpy()
    ...
    with open(output_file_path, mode='w', newline='') as file:
        ...
        for query_seqid, targets in predict_score_dict.items():
            for target_seqid, prediction in targets.items():
                writer.writerow([query_seqid, target_seqid, prediction.item()])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default=os.path.join(script_dir, 'models', 'model_tiny.pt'), help="Path to the the trained model")
    ...
    results_file = predict_step1(args.query_path, args.target_path, args.pairs_file, args.output_dir, args.esm_path, args.embedding_script, args.model_path)
    print(f"seq-seq action score results saved to: {results_file}")
