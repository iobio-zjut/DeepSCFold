...
from utils.util import get_pid_list, get_index_protein_dic
import numpy as np
from torch.nn.utils.rnn import pad_sequence
from utils.ConvNeXt import convnext_tiny
from utils.Retnet.retnet_fn2 import RetNet
...
class esm_ss_predict_tri_step2(nn.Module):
    ...
    def __init__(self, embed_dim=128, dim_1d=52, num_classes=1):
        ...
        self.convnext_model = convnext_tiny(num_classes, self.all_dim)
        self.retnet_model = RetNet(layers=4, hidden_dim=180, ffn_size=18, heads=4)

    def forward(self, x0, x1):
        x0_2d = x0.unsqueeze(2).permute(0, 3, 1, 2)
        x1_2d = x1.unsqueeze(1).permute(0, 3, 1, 2)
        x_2d = torch.matmul(x0_2d, x1_2d)
        s = self.convnext_model(x_2d)
        ss = torch.sigmoid(s)
